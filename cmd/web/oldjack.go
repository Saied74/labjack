...
// func (app *application) getU3Config() {
...
// 	if devHandle == nil {
// 		app.u3.Message = fmt.Sprintf("Couldn't open U3. Please connect one and try again")
// 		return
// 	}
//
...
// 		app.u3.Message = fmt.Sprintf("An error occurred when trying to read from the U3")
// 		C.LJUSB_CloseDevice(devHandle)
// 		return
// 	}
// 	// Check the command for errors
// 	if err := checkResponseForErrors(recBuffer); err != nil {
// 		C.LJUSB_CloseDevice(devHandle)
...
// }
//
// func (app *application) getSetPins(set bool) {
...
// 	}
// 	// Check the command for errors
// 	if err := checkResponseForBitErrors(recBuffer); err != nil {
// 		app.u3.Message = fmt.Sprintf("%v", err)
// 		C.LJUSB_CloseDevice(devHandle)
// 		return
// 	}
...
// }
//
// func checkResponseForBitErrors(recBuffer []byte) error {
// 	if recBuffer[0] == 0xB8 && recBuffer[1] == 0xB8 {
// 		// If the packet is [ 0xB8, 0xB8 ], that's a bad checksum.
// 		return fmt.Errorf("The U3 detected a bad checksum. Double check your checksum calculations and try again")
// 	} else {
...
// }
//
// func (app *application) checkResponse(op string, recBuffer []byte) error {
// 	if recBuffer[0] == 0xB8 && recBuffer[1] == 0xB8 {
// 		// If the packet is [ 0xB8, 0xB8 ], that's a bad checksum.
// 		return fmt.Errorf("The U3 detected a bad checksum. Double check your checksum calculations and try again")
// 	} else {
// 		if recBuffer[1] != app.srData[op].byte1 || recBuffer[2] != app.srData[op].byte2 || recBuffer[3] != app.srData[op].byte3 {
// 			// Make sure the command bytes match what we expect.
// 			return fmt.Errorf("Got the wrong command bytes back from the U3")
// 		}
//
// 		// Calculate the checksums.
// 		checksum16 := calculateChecksum16(recBuffer, app.srData[op].recLength)
// 		checksum8 := calculateChecksum8(recBuffer)
//
// 		if checksum8 != recBuffer[0] || int(recBuffer[4]) != checksum16&0xff || int(recBuffer[5]) != ((checksum16/256)&0xff) {
// 			// Check the checksum
